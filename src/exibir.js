...
async function loadDatas2(data3) {
  try {
    if (data3) {
      console.log('tenho');
...
async function loadDatas(data3) {
  try {
    ...
    console.log('tenho', `ss3 ${data3.jogador_nome}`);

    // Fazer requisições simultaneamente para melhorar o desempenho
    const [response, response2, response3] = await Promise.all([
      axios.get(`${apiUrl}/${data3.challengeId || data3}`),
      axios.get(apiUrl2),
      axios.get('http://127.0.0.1:8002/api/grupo2/pontuacoes/get-current-3p-player')
    ]);

    const datas = response.data;
    const groupedByRound = groupByRound(datas);
    updateTableBody(groupedByRound);
    updatePlayerDisplay(response3.data);
    updateTableHeader(data3);
  } catch (error) {
    console.error('Erro ao carregar datas:', error);
  }
}

// Função para agrupar os dados por round
function groupByRound(datas) {
  return datas.reduce((acc, data) => {
    acc[data.round_numero] = acc[data.round_numero] || [];
    acc[data.round_numero].push(data);
    return acc;
  }, {});
}
...
function updateTableBody(groupedByRound) {
  const datasTableBody = document.querySelector("#datasTable tbody");
  datasTableBody.innerHTML = "";
...
function updatePlayerDisplay({ player: jogador3, foto: jogador3Foto }) {
  console.log('nome da peça', jogador3Foto);

  const jogador = document.querySelector("#jogador");
  jogador.innerHTML = `<h2 class="player-name">${jogador3}</h2>`;

  const elementoFoto = document.querySelector('#jogadorFoto img');
  const fotoSrc = elementoFoto?.getAttribute('src') || "";
  const novoArquivo = jogador3Foto.split('/').pop();

  if (!fotoSrc.includes(novoArquivo)) {
    document.querySelector("#jogadorFoto").innerHTML = `
      <img src="http://127.0.0.1:8002/storage/${jogador3Foto}" 
           alt="Imagem do Jogador" class="player-image2">
    `;
  }
}


function updateRankinFoto(perfil) {
  console.log('Nome da peça:', perfil);

  const playerImageWrapper = document.querySelector("#player-image-wrapper");
  if (!playerImageWrapper) {
    console.error('Elemento #player-image-wrapper não encontrado.');
    return; // Sai da função se o elemento não existir
  }

  const elementoFoto = playerImageWrapper.querySelector('img');
  const fotoSrc = elementoFoto?.getAttribute('src') || "";
  const novoArquivo = perfil.split('/').pop();

  // Verifica se a nova foto é diferente da atual
  if (!fotoSrc.includes(novoArquivo)) {
    // Atualiza a imagem do jogador
    playerImageWrapper.innerHTML = `
      <img class="player-image" src="http://127.0.0.1:8002/storage/${perfil}" alt="Player">
    `;
    console.log('Imagem atualizada para:', perfil); // Log para confirmar a atualização
  } else {
    console.log('A imagem já está atualizada.');
  }
}



// Função para atualizar o cabeçalho da tabela
function updateTableHeader(data3) {
  const datasTrBody = document.querySelector("#datasTable thead");
  datasTrBody.innerHTML = `
    <tr>
      <th class="text-uppercase fs-4" colspan="3">${data3.jogador_nome}</th>
      <td class="text-end font-weight-bold fs-4 fw-bold">TOTAL DE PONTOS: ${data3.total_geral_pontos}</td>
    </tr>
    <tr>
      <th scope="col">POSIÇÃO</th>
      <th scope="col">ROUND</th>
...
async function loadPlayers(dados, it_round) {
  try {
    console.log('jogadores:', dados);

    const competicaoId = dados.competicaoId || 1;
    const response = await axios.get(`${apiUrl4}/${competicaoId}/${it_round}`);
    const players = response.data;

    const rankingAside = document.querySelector("#ranking");
    rankingAside.innerHTML = ""; // Limpa o conteúdo anterior

    players.forEach(player => createRankItem(player, rankingAside));
  } catch (error) {
    console.error('Erro ao carregar os dados dos jogadores:', error);
  }
}

// Função para criar e adicionar um item de ranking
function createRankItem(player, container) {
  const rankItem = document.createElement("div");
  rankItem.classList.add("rank-item");

  const profileImage = player.vc_profile || 
    'https://storage.googleapis.com/nbarankings-theringer-com-cms/public/media/ringerbasketballhub/players/NikolaJokic-small.png';

    updateRankinFoto(player.vc_profile)

  rankItem.innerHTML = `
    <div class="player-image-wrapper">
      <img class="player-image" src="http://127.0.0.1:8002/storage/${profileImage}" alt="Player">
    </div>
    <p class="jogador-info2">
      ${player.jogador_nome} | 
      <span>
...
ws.onmessage = async (event) => {
  try {
    const data = await extractDataFromEvent(event);

    const { it_challenge, it_round } = await fetchChallengeAndRound();

    if (it_challenge) {
      await handleParticipant(it_challenge, it_round);
    } 

    if (data?.it_id_participante) {
      await handleParticipant(data.it_id_participante, it_round);
    }
  } catch (error) {
...
// Função para extrair dados do evento WebSocket
async function extractDataFromEvent(event) {
  if (event.data instanceof Blob) {
    const text = await event.data.text();
    processMessage(text); // Processa a mensagem separadamente
...
// Função para obter os dados de challenge e round
async function fetchChallengeAndRound() {
  const [challengeRes, roundRes] = await Promise.all([
    axios.get(apiUrl6),
    axios.get(apiUrl5)
  ]);
  
  return {
    it_challenge: challengeRes.data.it_challenge,
    it_round: roundRes.data.it_round
  };
}

// Função para lidar com a lógica de participantes
async function handleParticipant(participantId, it_round) {
  try {
    const response = await axios.get(`${apiUrl3}/${participantId}`);
    const data = response.data;
    
    console.log('Dados do participante:', data);
    
    updateDOMWithData(data);
    await loadDatas(participantId);
    await loadPlayers(participantId, it_round);
    await updatePositions(participantId, it_round);
  } catch (error) {
...
function updateDOMWithData(data) {
  const datasTrBody = document.querySelector("#datasTable thead");
  datasTrBody.prepend(document.createTextNode(data));
}


async function updatePositions2(it_challenge, it_round) {
  try {
...
async function updatePositions(it_challenge, it_round) {
  try {
    ...
    const { data } = await axios.get(`${apiUrl7}/${it_challenge}/${it_round}`);
    
    // Reseta a estrutura existente e inicializa o score
    resetCircleContainer();
    inicializarScore(it_challenge, it_round);

    // Definição das posições e agrupamento de pontuações
    const positionScores = initializePositionScores();

    // Preenche as pontuações nas posições
    populatePositionScores(data, positionScores);

    // Gera a estrutura HTML e atualiza as cores dos círculos
    generateCircles(positionScores);
    updateCircleColors(positionScores);
...
// Função para inicializar o objeto de pontuações por posição
function initializePositionScores() {
  return {
    "Canto esquerdo (Corner Left)": [],
    "Ala esquerda (Wing Left)": [],
    "Special 2 points": [],
...
// Função para popular as pontuações nas posições
function populatePositionScores(data, positionScores) {
  data.forEach(({ posicao_nome, ponto, pontuacaoId }) => {
    if (positionScores[posicao_nome]) {
      positionScores[posicao_nome].push({ id: pontuacaoId, ponto });
    }
  });
}
